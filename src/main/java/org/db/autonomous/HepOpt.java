...
public class HepOpt {
  ...
  public  HepOpt() throws IOException {
    ...
    updateSelectBitmap();
    this.converter = new RelToSqlConverter(MysqlSqlDialect.DEFAULT);
  }
  public void initrule2classidx() throws IOException {
    InputStream inputStream = getClass().getResourceAsStream("/rules_for_selected/standard.txt");
  ...
  public void updateSelectBitmap() throws IOException {
    InputStream inputStream =  getClass().getResourceAsStream("/rules_for_selected/user_selected_rules.txt");
    BufferedReader bufReader = new BufferedReader(new InputStreamReader(inputStream));
    String line = null;
    while(!Objects.equals(line = bufReader.readLine(), null)){
  ...
  public List findBest(RelNode relNode){
    ...
    res.add(finalNode);
    String new_sql = converter.visitRoot(finalNode).asStatement().toSqlString(MysqlSqlDialect.DEFAULT).getSql();
    res.add(new_sql);
